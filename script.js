...
// Generate the bot's response (via your backend)
const generateResponse = async (botMsgDiv) => {
  ...
  try {
    ...
    const responseText = data.candidates?.[0]?.content?.parts?.[0]?.text?.replace(/\*\*([^*]+)\*\*/g, "$1").trim() || "No response received.";
...
// Smooth horizontal scroll for suggestions
